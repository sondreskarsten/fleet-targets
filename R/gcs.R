...
read_gcs <- function(path, columns = NULL, bucket = BUCKET) {
  local <- download_gcs(path, bucket)
  if (is.null(columns)) arrow::read_parquet(local) else arrow::read_parquet(local, col_select = dplyr::all_of(columns))
}
...
restore_targets_meta <- function() {
  ...
  objs <- tryCatch(googleCloudStorageR::gcs_list_objects(prefix = "fleet_targets/_meta/"), error = function(e) data.frame(name = character(0)))
  if (nrow(objs) == 0) return(0L)
  for (nm in objs$name) {
    tryCatch(googleCloudStorageR::gcs_get_object(nm, saveToDisk = file.path(meta_dir, basename(nm)), overwrite = TRUE), error = function(e) NULL)
  }
  nrow(objs)
}
